...
export const cuttingJobItems = pgTable('cutting_job_items', {
    id: uuid('id').primaryKey().defaultRandom(),
    cuttingJobId: uuid('cutting_job_id').notNull().references(() => cuttingJobs.id, { onDelete: 'cascade' }),
    orderItemId: uuid('order_item_id').notNull().references(() => orderItems.id),
    quantity: integer('quantity').notNull(),
    createdAt: timestamp('created_at').defaultNow().notNull(),
    updatedAt: timestamp('updated_at').defaultNow().notNull(),
}, (table) => [
    unique('cutting_job_item_unique').on(table.cuttingJobId, table.orderItemId)
]);
