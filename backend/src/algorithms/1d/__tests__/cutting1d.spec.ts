import { optimize1D, CuttingPiece1D, StockBar1D, Optimization1DOptions } from '../cutting1d';
...
describe('1D Cutting Algorithm', () => {
    ...
    describe('Best Fit Decreasing (BFD)', () => {
        it('should be selected via options', () => {
            const pieces: CuttingPiece1D[] = [{ id: 'p1', length: 100, quantity: 1, orderItemId: 'o1' }];
            const stock: StockBar1D[] = [{ id: 's1', length: 1000, available: 1 }];

            // We can't easily spy on the internal function export, but we can verify it runs and produces valid output
            const result = optimize1D(pieces, stock, { ...defaultOptions, algorithm: 'BFD' });
            expect(result.success).toBe(true);
        });
