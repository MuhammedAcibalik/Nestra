...
import { createActiveSheet, tryPlaceBottomLeft } from './sheet-manager';
...
export function bottomLeftFill(
    ...
    for (const piece of expandedPieces) {
        ...
        for (const sheet of activeSheets) {
            if (tryPlaceBottomLeft(sheet, piece, options)) {
                placed = true;
                break;
            }
        }
        ...
        if (!placed) {
            ...
            for (const orient of orientations) {
                ...
                if (availableStock) {
                    const newSheet = createActiveSheet(
                        availableStock.id,
...
export const BOTTOM_LEFT_ALGORITHM = {
    name: '2D_BOTTOM_LEFT',
    type: '2D' as const,
