...
export interface IDashboardService {
    ...
    getMaterialUsage(): Promise<{ materialType: string; usageCount: number; wastePercentage: number }[]>;
}
...
export class DashboardService implements IDashboardService {
    ...
    async getWasteAnalytics(days = 30): Promise<IWasteAnalytics[]> {
        ...
        // Group by week
        const weeklyData = new Map<string, { totalWaste: number; wastePercentages: number[]; count: number }>();
    ...
    async getMaterialUsage(): Promise<{ materialType: string; usageCount: number; wastePercentage: number }[]> {
