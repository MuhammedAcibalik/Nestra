...
export class OptimizationFacade {
    ...
    async createScenario(
        ...
        try {
            ...
            const createInput = {
                ...
                parameters: (data.parameters ?? {}) as { objectives?: object; constraints?: object },
    ...
    async getScenarios(filter?: IScenarioFilterDto): Promise<IResult<IScenarioDto[]>> {
        try {
    ...
    async comparePlans(planIds: string[]): Promise<IResult<IPlanComparisonDto[]>> {
        try {
            ...
            const comparisons: IPlanComparisonDto[] = validPlans.map((plan, index) => ({
                planId: plan.id,
                planNumber: plan.planNumber,
    ...
    private parseParameters(scenario: ScenarioWithRelations): IOptimizationParameters {
        ...
        return {
            ...
            selectedStockIds: scenario.selectedStockIds as string[]
        };
    }
    ...
    private toScenarioDto(scenario: ScenarioWithRelations): IScenarioDto {
        return {
            ...
            parameters: scenario.parameters as object,
    ...
    private parseLayoutData(data: unknown): ICuttingPlanDto['layouts'][0]['layoutData'] {
        ...
        return data as ICuttingPlanDto['layouts'][0]['layoutData'];
    }

    private handleError(error: unknown, code: string): IResult<never> {
