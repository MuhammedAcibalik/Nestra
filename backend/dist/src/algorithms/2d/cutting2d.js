 ...
 */
function canPlace(rect, sheetDims, placements, kerf) {
    // Check bounds
    ...
    const newRectWithKerf = { x: rect.x, y: rect.y, width: rect.width + kerf, height: rect.height + kerf };
 ...
 */
function findBottomLeftPosition(sheet, dims, kerf) {
    // Generate candidate positions (corners of existing placements + origin)
    const candidates = [{ x: 0, y: 0 }];
    for (const p of sheet.placements) {
        candidates.push({ x: p.x + p.width + kerf, y: p.y }, { x: p.x, y: p.y + p.height + kerf }, { x: p.x + p.width + kerf, y: p.y + p.height + kerf });
    }
 ...
 */
function tryPlaceInActiveSheetsBL(activeSheets, piece, options) {
    for (const sheet of activeSheets) {
        const orientations = getOrientations(piece, options.allowRotation);
        for (const orient of orientations) {
            const pos = findBottomLeftPosition({ width: sheet.width, height: sheet.height, placements: sheet.placements }, orient, options.kerf);
            if (pos) {
 ...
 */
function tryCreateNewSheetBL(sortedStock, stockUsage, activeSheets, piece, options) {
 ...
 */
function tryPlaceGuillotineInSheet(sheet, piece, kerf, allowRotation) {
 ...
 */
function initializeGuillotineSheet(stock, piece, orient, kerf) {
    const newSheet = {
        stockId: stock.id,
        width: stock.width,
        height: stock.height,
 ...
 */
function tryCreateNewSheetGuillotine(sortedStock, stockUsage, activeSheets, piece, options) {
    for (const stock of sortedStock) {
        const remaining = stockUsage.get(stock.id);
        if (remaining <= 0)
            continue;
 ...
 */
function buildResults(activeSheets, expandedPieces, unplacedPieces) {
