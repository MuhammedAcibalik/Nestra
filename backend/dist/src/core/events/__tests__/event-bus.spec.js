...
describe('EventBus', () => {
    let eventBus;
    beforeEach(() => {
        event_bus_1.EventBus.reset();
        eventBus = event_bus_1.EventBus.getInstance();
    });
    it('should be a singleton', () => {
        const instance1 = event_bus_1.EventBus.getInstance();
        const instance2 = event_bus_1.EventBus.getInstance();
        expect(instance1).toBe(instance2);
    });
    it('should publish and subscribe to events', async () => {
        const eventType = 'test.event';
        const payload = { data: 'test' };
        const event = (0, event_bus_1.createDomainEvent)(eventType, 'Test', '1', payload);
    ...
    it('should handle multiple subscribers for same event', async () => {
        const eventType = 'test.event';
        const event = (0, event_bus_1.createDomainEvent)(eventType, 'Test', '1', {});
    ...
    it('should log events', async () => {
        const event = (0, event_bus_1.createDomainEvent)('test.event', 'Test', '1', {});
        await eventBus.publish(event);
