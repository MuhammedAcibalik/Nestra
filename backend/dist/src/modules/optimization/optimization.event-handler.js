...
class OptimizationEventHandler {
    ...
    register() {
        ...
        eventBus.subscribe(events_1.EventTypes.PLAN_STATUS_UPDATE_REQUESTED, this.handlePlanStatusUpdateRequested.bind(this));
        // Handle production started - could trigger notifications
        eventBus.subscribe(events_1.EventTypes.PRODUCTION_STARTED, this.handleProductionStarted.bind(this));
    ...
    async handlePlanStatusUpdateRequested(event) {
        ...
        try {
            ...
            await eventBus.publish(events_1.DomainEvents.planStatusUpdated({
    ...
    unregister() {
        const eventBus = events_1.EventBus.getInstance();
        eventBus.unsubscribe(events_1.EventTypes.PLAN_STATUS_UPDATE_REQUESTED);
        eventBus.unsubscribe(events_1.EventTypes.PRODUCTION_STARTED);
