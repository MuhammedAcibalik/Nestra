...
class BarcodeGenerator {
    ...
    generateQR(data, opts) {
        ...
        for (let row = 0; row < moduleCount; row++) {
            for (let col = 0; col < moduleCount; col++) {
                ...
                if (this.isFinderPatternArea(row, col, moduleCount))
                    continue;
    ...
    drawFinderPattern(x, y, moduleSize, color) {
        let pattern = '';
        // Outer square (7x7)
 ...
 * Generate label with barcode
 */
function generateLabel(data, options) {
    const generator = new BarcodeGenerator();
